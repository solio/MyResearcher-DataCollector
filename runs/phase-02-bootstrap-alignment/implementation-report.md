...
After worktree recovery, the branch is based on `main`; no Batch implementation,
target config, Batch artifact or batch-only test is included. The shared
`cli/main.py` diff contains only the Bootstrap persistent single-stock command.
